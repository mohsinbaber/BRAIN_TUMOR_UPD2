...
def search_data():
    ...
    if (search.get().startswith("200")):
        ...
        messagebox.showerror("Error", "Doctor ID must always starts with initials 200")
...
def on_enter(e):
    updBtn['background'] = '#F0F0F0'

def on_leave(e):
    updBtn['background'] = '#CBCBCB'
#function upd_img
updBtn = Button(root,text="Update Image",width=20,height=2,background="#CBCBCB", command=upd_img)
updBtn.place(x=300, y=575)
updBtn.bind("<Enter>",on_enter)
updBtn.bind("<Leave>",on_leave)

def on_enter(e):
    show_res['background'] = '#F0F0F0'

def on_leave(e):
    show_res['background'] = '#CBCBCB'

show_res = Button(root,text="Show Result",width=20,height=2,background="#CBCBCB", command=show_result)
show_res.place(x=500, y=575)
show_res.bind("<Enter>",on_enter)
show_res.bind("<Leave>",on_leave)
...
def update_data():
    try:
        ...
        if(ans == True):
            ...
            if (search.get().startswith("200") == True):
                ...
                messagebox.showerror("Error", "Patient ID must always start with 200")
    except:
        messagebox.showerror("", "Technical error occured")

#function update_data
def on_enter(e):
    updateBtn['background'] = '#F0F0F0'

def on_leave(e):
    updateBtn['background'] = '#CBCBCB'
updateBtn = Button(root, text="Update Data", height=2,width=20, font=(None,10),background="#CBCBCB",command=update_data)
updateBtn.place(x=1020, y=576)
updateBtn.bind("<Enter>",on_enter)
updateBtn.bind("<Leave>",on_leave)
...
def disable_event():
    if(messagebox.askyesno("Exiting","Do you wish to exit? \nAny unsaved data will be lost!")):
        root.destroy()
