...
def search_data():
    ...
    if (search.get().startswith("100")):
        ...
        for row in result:
            ...
            monVar.set(month)
            yearVar.set(year)
            cal.insert(END,row[8])
    else:
        messagebox.showerror("Error", "Doctor ID must always starts with 100")
            ...
            if(len(passwordText.get())==0):
                if (validateContact(contactText.get("1.0", "end-1c")) == True and len(contactText.get("1.0", "end-1c")) == 11 and (contactText.get("1.0", "end-1c").startswith("03") == True or contactText.get("1.0","end-1c").startswith("0423"))):
                    if(calyear >= 30 and calyear <= 65):
                            ...
                            val = (fnameText.get("1.0","end-1c"),lnameText.get("1.0","end-1c"),addressText.get("1.0","end-1c"),contactText.get("1.0","end-1c"),str(tkvar.get()),desText.get("1.0","end-1c"),dob,str(cal.get_date()),search.get())
                            cursor.execute(sql,val)
                            conn.commit()
                    ...
                    if (validateContact(contactText.get("1.0", "end-1c")) == True and len(contactText.get("1.0", "end-1c")) == 11 and (contactText.get("1.0", "end-1c").startswith("03") == True or contactText.get("1.0","end-1c").startswith("0423"))):
                        if (calyear >= 30 and calyear <=65):
                                if (len(passwordText.get()) >= 8):
                                    ...
                                    desText.get("1.0", "end-1c"),passwordText.get(),dob,str(cal.get_date()),search.get())
...
updateBtn = Button(root, text="Update", height=2,width=20, font=(None,10),background="#CBCBCB", command=update_data)
